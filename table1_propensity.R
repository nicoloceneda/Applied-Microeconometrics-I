...
dvals = c(2, 5, 10, 15, 20, 30, 40)                                             # Number of features {d}
sigma = 1                                                                       # Volatility {sigma}

ntree = 1000                                                                    # Number of trees {B}
simu.reps = 500                                                                 # Number of simulations
...
# - Treatment effect fixed tau(x) = 0
# - Test ability of method to resist bias due to interaction between e(x) and m(x)
# - Since the goal is accurate propensity matching we use propensity trees (procedure 2)

simu.fun = function(d) {
  
  # Generate X
  X = matrix(runif(n * d, 0, 1), n, d)                                          # Matrix of features {X ~ U([0,1])}
  X.test = matrix(runif(n.test * d, 0, 1), n.test, d)                           # Matrix of test features
  
  # Generate W
  e = apply(X, 1, propensity)                                                   # Treatment propensity in (27)
  W = rbinom(n, 1, e)                                                           # Treatment: prob. of exactly 1 success in n indep. Bernoulli trials
  
  # Generate Y
  Y = apply(X, 1, baseline) + sigma * rnorm(n)                                  # Vector of outcomes {Y ~ N(E[Y|X],1)}
  
  # Random forest
  forest = propensityForest(X, Y, W, 
                            num.trees = ntree,                                  # Number of trees {B}
                            sample.size = n / 10,                               # Sample size {s}
  ...
  k.small = 10
  knn.0.mu = knn.reg(X[W==0,], X.test, Y[W==0], k = k.small)$pred               # k Nearest Neighbor Regression
