...
dvals = c(2, 5, 10, 15, 20, 30, 40)                                             # Number of features
...
propensity = function(x) {                                                      # Density of beta function
  0.25 * (1 + dbeta(x[1], 2, 4))                                                # - x: vector of quantiles
}

simu.fun = function(d) {
  
  X = matrix(runif(n * d, 0, 1), n, d)                                          # Features matrix
  e = apply(X, 1, propensity)                                                   # Apply propensity to each row of features matrix
